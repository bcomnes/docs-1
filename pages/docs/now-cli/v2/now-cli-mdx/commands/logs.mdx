...
<Note type="warning">
  This command is for <b>advanced use only</b>. We recommend using the
  deployment overview page on the ZEIT dashboard to view logs.
</Note>
...
The `--since` option, can be used to return logs only after a specifc date, using the ISO 8601 format.
...
The `--until` option, can be used to return logs only up until a specifc date, using the ISO 8601 format.
